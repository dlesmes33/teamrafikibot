                ...
                if texto == "/cancelar" or texto == "/cancelar@teamrafikibot":
                    ...
                    if not texto in lista_un and servicio.validar_nombreUsuario(texto):
                        ...
                        enviar_mensaje(info.id_chat,
                                       "Ese usuario ya esta registrado intentelo de nuevo o escriba /cancelar para salir")

                elif paso == "2":
                    if not texto in lista:

                        servicio.set_variable("p2", texto)
                        enviar_mensaje(info.id_chat, "Va ha hacer el registro de:\nusuario: " + servicio.get_variable(
                            "p1") + "\nid_telegram:" +
                                       servicio.get_variable(
                    ...
                    if not num == -1:
                        ...
                        enviar_mensaje(info.id_chat, "Va a realizar un prestamo desde: " + servicio.get_variable(
                            "p1") + " hacia: " + servicio.get_variable(
                            "p2") + " por un valor de " + servicio.get_variable(
                            "p3") + "\n" + "Presione cualquier cosa para continuar o /cancelar para salir")
                        servicio.set_variable("paso", "5.6")
                    else:
                        enviar_mensaje(info.id_chat,
                                       "El numero debe ser positivo y '.'para marcar los decimales intente de nuevo o /cancelar para salir")

                elif paso == "5.6":
                    servicio.prestar(servicio.get_variable("p1"), servicio.get_variable("p2"),
                                     float(servicio.get_variable("p3")))
                    ...
                    servicio.insertar_paquete(servicio.get_variable("p1"), servicio.get_variable("p2"),
                                              servicio.get_variable("p3"))
                    enviar_mensaje(info.id_chat, "Operacion realizada")
                    servicio.set_variable("paso", "0")
